...
class GroupChatPage extends StatefulWidget {
  ...
  GroupChatPage({required this.groupId, required this.user});
  ...
  void _handleSendPressed(types.PartialText message) async {
    ...
    final textMessage = types.TextMessage(
      author: widget.user.copyWith(
        ...
        metadata: {'role': widget.user.metadata?['role'] ?? 'teacher'},
      ),
            ...
            onPressed: () async {
              final confirm = await showDialog(
                ...
                builder: (context) => AlertDialog(
                  title: Text('确认删除'),
                  content: Text('确定要删除该群聊吗？此操作无法撤销。'),
                  actions: [
                    TextButton(
              ...
              if (confirm == true) {
                await groupChatManager.deleteGroupChat(widget.groupId);
                Navigator.pop(context); // 删除后返回上一页
              }
            },
          ),
        ],
