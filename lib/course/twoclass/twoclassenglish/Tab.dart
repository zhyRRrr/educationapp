...
import '../.././twoclass/twoclassenglish/KeC.dart'; // 导入KeC.dart文件

void main() {
  runApp(Twoclassenglish());
}

class Twoclassenglish extends StatefulWidget {
  @override
  _TwoclassenglishState createState() => _TwoclassenglishState();
}

class _TwoclassenglishState extends State<Twoclassenglish> {
  ...
  Future<void> _loadLastLearned() async {
    ...
    setState(() {
      _lastLearn = prefs.getString('video7_last_learned_title') ?? '';
      _lastLearnedIndex = prefs.getInt('video7_last_learned_index') ?? -1;
  ...
  void _updateLastLearned(String title, int index) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('video5_last_learned_title', title);
    await prefs.setInt('video5_last_learned_index', index);
  }
