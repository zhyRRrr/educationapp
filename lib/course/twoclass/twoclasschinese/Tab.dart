...
import '../.././twoclass/twoclasschinese/KeC.dart'; // 导入KeC.dart文件

class Twoclasschinese extends StatefulWidget {
  @override
  _TwoclasschineseState createState() => _TwoclasschineseState();
}

class _TwoclasschineseState extends State<Twoclasschinese> {
  ...
  Future<void> _loadLastLearned() async {
    ...
    setState(() {
      _lastLearned = prefs.getString('video13_last_learned_title') ?? '';
      _lastLearnedIndex = prefs.getInt('video13_last_learned_index') ?? -1;
  ...
  void _updateLastLearned(String title, int index) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('video13_last_learned_title', title);
    await prefs.setInt('video13_last_learned_index', index);
  }
