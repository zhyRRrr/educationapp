import '../.././twoclass/twoclassmath/HomeWork.dart';
...
class TwoMathclass extends StatefulWidget {
  @override
  _TwoMathclassState createState() => _TwoMathclassState();
}

class _TwoMathclassState extends State<TwoMathclass> {
  ...
  Future<void> _loadLastLearned() async {
    ...
    setState(() {
      _lastLearn = prefs.getString('video14_last_learned_title') ?? '';
      _lastLearnedIndex = prefs.getInt('video14_last_learned_index') ?? -1;
  ...
  void _updateLastLearned(String title, int index) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('video14_last_learned_title', title);
    await prefs.setInt('video14_last_learned_index', index);
  }
