...
void main() {
  runApp(Oneclasschinese1());
}

class Oneclasschinese1 extends StatefulWidget {
  @override
  _OneclasschineseState createState() => _OneclasschineseState();
}

class _OneclasschineseState extends State<Oneclasschinese1> {
  ...
  Future<void> _loadLastLearned() async {
    ...
    setState(() {
      _lastLearn = prefs.getString('video15_last_learned_title') ?? '';
      _lastLearnedIndex = prefs.getInt('video15_last_learned_index') ?? -1;
  ...
  void _updateLastLearned(String title, int index) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('video15_last_learned_title', title);
    await prefs.setInt('video15_last_learned_index', index);
  }
